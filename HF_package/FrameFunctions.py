...
def image_finder(cornersDF, polygon_coords):
    ...
    for image in range(0, len(cornersDF.camera)):
        ...
        # select more stringently to reduce processing time
        ...
        # # select more stringently to reduce processing time
        # if min(img_x)+4.25 < poly_x_min < max(img_x)-4.25 and min(img_x)+4.25 < poly_x_max < max(img_x)-4.25 and \
        #         min(img_y)+4 < poly_y_min < max(img_y)-4 and min(img_y)+4 < poly_y_max < max(img_y)-4:
        #     img_of_interest.append([cornersDF.loc[image].camera, image])

    return img_of_interest


def image_finder2(cornersDF, polygon_coords):
        ...
        if utils._check_geojson(geoj):
            ...
            with open("{path_geo}/{pic_n}".format(path_geo=path_geojson_current, pic_n=geoj),
                ...
                for polygon in polygons:
                    ...
                    if len(coords) != 5:  # apparently, not all geojsons have the same format ?! e.g., Bolli 20200312.
